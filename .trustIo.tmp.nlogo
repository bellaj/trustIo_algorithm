...
globals [
  ...
  current_ticks ;; current iteration of Go
]
...
;;turtles-own can only be used at the beginning of a program, before any function definitions. It defines the variables belonging to each turtle
...
to setup
  ...
  let n_malicious round ((number_peers * malicious_peers) / 100) ; malicious_peers % defined in the interface
  ...
  set number_of_nodes number_peers ;number_peers is defined in the interface
...
;; iterated forever as the buton forever property is selected
to go
...
;  ]
  ...
  layout
   ...
   if remove_add_allowed ; remove and add a peer in the same time =>it could be seen as good turns into malicious with 40%probability
  [
  ;; it depends on the removal or adding rate defined in the interface

  ;;remove a random peer with the pre-defined rate
  remove-random-peer

  ;;add a random peer with the pre-defined rate
  add-random-peer
  ; print("node added")
  ]
  ;;resize-nodes
...
;;this method adds a new peer to the network => ADD manually a peer from the interface
to add
  crt 1
  [
    set size 1.2
...
to collaborate ; chose randomly a service and then request the most trusted peer
    ...
    evaluate-current-connections peer1 ;;;;evaluates all the connections of a peer. the link is removed if the nodes trust value is below the threshold
    ...
    [
      ...
      let list_counter 0
    ifelse choose_random_or_most_trusted
    [
...
to perform-transaction [peer1 peer2]
  ...
  [
    ...
    [
      ...
      set list_satisfaction lput get-satisfaction self peer2 peer1_act_maliciously peer2_act_maliciously list_satisfaction ; peer2 will give feedback for peer1's actions and add it to its list of satisfaction
      ...
      ;lput Adds value to the end of a list and reports the new list. set mylist lput 42 mylist
...
to initialize-turtle-lists [peer]
  ...
  set list_credibility [] ; Cr(P(u,i))
  set list_other_peer_id []
  set list_time_of_transaction []
  set list_transaction_context_factor_value [] ;TF(u,i)
...
to-report get-binary-satisfaction [peer1 peer2 peer1_acts_malicious peer2_acts_malicious]
  ...
  ifelse peer1_acts_malicious ;if true
  [
  ...
  ifelse peer2_acts_malicious ;if true = peer2 acts maliciously
  [
      ;;dont collaborate
      ifelse performance_quality = 0 ; IMO satifyed by bad content
      [
        set satisfaction 1
      ]
      [
  ...
  [
    ...
    [
      ...
      [ ?1 ->
        ...
        let transaction_total precision (?1 * temp_credibility * temp_transaction_context_factor) 3
...
to-report calculate-community-context-factor [peer time_ticks]
  ...
  [
    ...
    [
      ...
      [ ?1 ->
        if (time_ticks > current_ticks OR ?1 >= (current_ticks - time_ticks))
        [
...
[
 ...
 [
   ...
   [ ?1 ->
     ...
     if (time_ticks > current_ticks OR transaction_time >= (current_ticks - time_ticks)) AND not (turtle ?1 = NOBODY)
     [
       ask turtle ?1
       [
        ...
        [ ?1 ->
          ...
          [
            ...
            [
              ...
              let transaction_total precision (alpha *(?1 * (temp_credibility / sum_other_peers_trust_values) * temp_transaction_context_factor)) 3
